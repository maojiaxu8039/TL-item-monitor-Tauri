#!/usr/bin/env python3
"""
SS13 day 1-5 历史数据补采脚本 - 刷图小助手 /price/history 版

策略:
- 刷图小助手 /price/history?season_id=1501&item_id=X&range=season 返回整个赛季小时历史
- 对每个 SS13 item_id 调一次（2077 个物品）
- 按小时写入 item_snapshots_ss13_normal（INSERT OR IGNORE）
- 7/17 开服当天数据可能缺失（API 从 7/18 02:00 开始）
...
# 复用 /get?season_id=1501 拿到的物品清单
ITEM_LIST_FILE = "/tmp/luosi_ss13.json"
...
# 刷图小助手返回的 ts 已经是整点小时秒（1777114800, 1777118400 ... 间隔 3600）
# 直接使用，无需转换
...
for item_id, points in results.items():
    ...
    for pt in points:
        ...
        day = season_day_for_ts(ts, SS13_START)
...
SQL_FILE = "/tmp/ss13_backfill_luosi.sql"
