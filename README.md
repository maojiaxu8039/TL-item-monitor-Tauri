...
- 默认严格校验 HTTPS 证书；仅在显式设置 `TL_SKIP_CERT_VERIFY=1` 时临时跳过，并输出安全告警
- 螺蛳粉上游默认仅提供 HTTP；桌面端可通过 `TL_LUOSI_API_URL=https://你的代理地址/get` 切换到 HTTPS 反向代理，服务端可在 `api_endpoints.luosi` 中配置代理地址
- 建议生产环境使用反向代理和 HTTPS
