...
commands = [
    ...
    ('[4/5] Copy binary to host backup directory', 'sudo mkdir -p /tmp/tl-monitor-backup && sudo cp tl-monitor-server /tmp/tl-monitor-backup/tl-monitor-server'),
    ('[5/5] Run new container', 'sudo docker run -d --name tl-monitor-server --restart unless-stopped -p 38457:8080 -v /data_s001/data/udata/real/15510607744/Docker/tl-monitor/data:/app/data -v /data_s001/data/udata/real/15510607744/Docker/tl-monitor/config:/app/config -e TZ=Asia/Shanghai debian:stable-slim bash -c "cp /tmp/tl-monitor-backup/tl-monitor-server /app/tl-monitor-server && chmod +x /app/tl-monitor-server && mkdir -p /app/data /app/config && cd /app && ./tl-monitor-server"'),
]
