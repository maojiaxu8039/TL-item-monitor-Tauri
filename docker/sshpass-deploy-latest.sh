...
commands = [
    ...
    ('[3/5] Create directories and set permissions', 'sudo mkdir -p data config && sudo chmod -R 777 data config'),
    ('[4/5] Ensure binary has execute permission', 'sudo chmod 755 tl-monitor-server'),
    ('[5/5] Run container with volume mount', 'sudo docker run -d --name tl-monitor-server --restart unless-stopped -p 38457:8080 -v /data_s001/data/udata/real/15510607744/Docker/tl-monitor:/app debian:stable-slim sh -c "chmod +x /app/tl-monitor-server && cd /app && ls -la && ./tl-monitor-server"'),
]
