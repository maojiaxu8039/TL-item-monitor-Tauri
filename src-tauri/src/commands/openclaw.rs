...
pub async fn openclaw_chat(
    ...
    while start_time.elapsed() < challenge_timeout && !challenge_received && !auth_sent {
                ...
                if let tokio_tungstenite::tungstenite::Message::Text(text) = msg {
                    ...
                    if let Ok(event) = serde_json::from_str::<ChallengeEvent>(&text) {
                        ...
                        if event.event == "connect.challenge" {
                            ...
                            write.send(tokio_tungstenite::tungstenite::Message::Text(auth_response.into()))
                                .await
    ...
    if !auth_sent {
        ...
        write.send(tokio_tungstenite::tungstenite::Message::Text(auth_msg.into()))
            .await
            .map_err(|e| format!("发送认证失败: {}", e))?;
        auth_sent = true;
        tokio::time::sleep(Duration::from_millis(500)).await;
    ...
    write.send(tokio_tungstenite::tungstenite::Message::Text(request_json.into()))
        .await
                ...
                match msg {
                    tokio_tungstenite::tungstenite::Message::Text(text) => {
                        ...
                        if let Ok(data) = serde_json::from_str::<serde_json::Value>(&text) {
                            if let Some(content) = data.get("content").or(data.get("text")).or(data.get("response")) {
